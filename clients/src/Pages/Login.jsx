...
const Login = () => {
    ...
    try {
      ...
      if (state === 'Sign Up') {
        ...
        if (data.success) {
          ...
          if (!data.isEmailVerified) {
            toast.info('Please verify your email before logging in.')
            navigate('/verify-email') // Redirect to the OTP page
            // Send OTP after redirection to OTP verification page
            sendOTPToUser(email); // Call this function to trigger OTP sending.
          } else {
            navigate('/') // Navigate to the home page if email is verified
          }
        } else {
  ...
  const sendOTPToUser = async (email) => {
    try {
      // Send a request to the backend to send OTP
